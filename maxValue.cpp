...
// This program makes random numbers and finds the biggest
...
int LargestNumber(std::array<int, N> listOfNumbers) {
    ...
    for (int i = 0; i < listOfNumbers.size(); i++) {
        if (listOfNumbers[i] > listOfNumbers[largestI]) {
            largestI = i;
        }
    }
...
main() {
    ...
    // call functions
    largestIndex = LargestNumber(randomNumbers);
    // output
    std::cout << "The largest number is: " << randomNumbers[largestIndex];
}
